...
def main() -> None:
    ...
    parser.add_argument('--algorithm', choices=['hierarchical_baseline', 'hierarchical_weighted', 'sdgca', 'sdgca_modified'], default='hierarchical_weighted')
    ...
    parser.add_argument('--out', default=ROOT / 'results' / 'runs_stability.tsv')
    ...
    if args.algorithm == 'hierarchical_baseline':
        ...
        run_kwargs = {'nwca_para': params['lambda_'], 'eta': params['eta'], 'theta': params['theta'], 'diffusion_time': params['diffusion_time']}
    ...
    print(f'Прогон {args.algorithm} на {dataset_path.name}, runs={args.max_runs}, m={args.m}')
    result = runner(dataset_path=dataset_path, data_name=args.dataset, seed=args.seed, m=args.m, cnt_times=args.max_runs, method=args.method, **run_kwargs)
    ...
    if nmi_scores is None:
        ...
        for i in range(args.max_runs):
            r = runner(dataset_path=dataset_path, data_name=args.dataset, seed=args.seed + i, m=args.m, cnt_times=1, method=args.method, **run_kwargs)
    ...
    for k in [5, 10, 20, 30, 50]:
        ...
        rows.append({'runs': k, 'nmi_mean': round(mean, 4), 'nmi_std': round(std, 4), 'ci_low': round(lo, 4), 'ci_high': round(hi, 4), 'ci_width': round(hi - lo, 4)})
    ...
    try:
        ...
        ax.set_ylabel('NMI')
        ax.set_title(f'Стабилизация среднего NMI с ростом runs\n({args.algorithm}, {args.dataset}, {args.method})')
