"""Select one global consensus profile on validation datasets only."""
...
def main() -> None:
    ...
    df["profile_id"] = (
        ...
        .fillna("")
        .map(str)
        .agg(lambda row: "|".join(row.to_list()), axis=1)
    )
    ...
    if args.metric == "rank":
        ...
        summary = (
            ...
            .reset_index()
            .sort_values(["mean_rank", "mean_nmi", "mean_runtime"], ascending=[True, False, True])
        )
    else:
        metric_column = {
            "NMI": "mean_nmi",
        ...
        summary = (
            ...
            .reset_index()
            .sort_values([metric_column, "mean_runtime"], ascending=[False, True])
        )
    ...
    profile.update(
        {
            ...
            "mean_validation_nmi": float(best["mean_nmi"]),
            "mean_validation_ari": float(best["mean_ari"]),
            "mean_validation_f_score": float(best["mean_f"]),
            "mean_validation_runtime_sec": float(best["mean_runtime"]),
            "validation_dataset_count": int(best["datasets"]),
            "note": "Global profile selected on dataset-level validation split. Do not tune on test.",
        }
    )
