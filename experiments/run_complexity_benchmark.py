...
def make_dataset(n: int, k: int, d: int, seed: int) -> Path:
    ...
    pts_per = n // k
    x = np.vstack([centres[i] + rng.normal(scale=0.6, size=(pts_per, d)) for i in range(k)])
...
def main() -> None:
    ...
    parser.add_argument('--sizes', type=int, nargs='+', default=[300, 600, 1000, 1500, 2000, 3000])
    ...
    parser.add_argument('--algorithms', nargs='+', default=['hierarchical_baseline', 'hierarchical_weighted', 'sdgca', 'sdgca_modified'])
    parser.add_argument('--out', default=ROOT / 'results' / 'complexity_benchmark.tsv')
    ...
    runners = {'hierarchical_baseline': (run_hierarchical_consensus, {}), 'hierarchical_weighted': (run_weighted_hierarchical_consensus, {}), 'sdgca': (run_sdgca, {}), 'sdgca_modified': (run_sdgca_modified, 'resolve')}
    rows: list[dict] = []
    for n in args.sizes:
        ...
        for alg in args.algorithms:
            ...
            if kwargs == 'resolve':
                ...
                kwargs = {'nwca_para': p['lambda_'], 'eta': p['eta'], 'theta': p['theta'], 'diffusion_time': p['diffusion_time']}
            t0 = time.time()
            try:
                res = runner(dataset_path=path, data_name=path.stem, seed=args.seed, m=args.m, cnt_times=args.runs, method=args.method, **kwargs)
                status = 'ok'
                err = ''
                ...
                nmi = float('nan')
            t1 = time.time()
            rows.append({'n': n, 'algorithm': alg, 'seconds': round(t1 - t0, 3), 'nmi_mean': round(nmi, 4), 'method': args.method, 'm': args.m, 'runs': args.runs, 'status': status, 'error': err})
