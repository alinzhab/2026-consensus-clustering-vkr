"""Full benchmark on real biomedical datasets.

Compares for each dataset:
  - base_members : mean NMI of pre-built ensemble (already in .npz)
  - kmeans       : sklearn KMeans (5 independent runs, best K known)
  - ward         : sklearn AgglomerativeClustering with Ward linkage
  - hierarchical_baseline / hierarchical_weighted / sdgca / sdgca_modified

Output: results/benchmark_real_datasets.tsv  (incremental, resume-safe)
"""
...
# ── config ─────────────────────────────────────────────────────────────────────
RUNS    = 3          # consensus algorithm independent runs
M       = 20         # ensemble size per run
METHOD  = "ward"     # linkage for hierarchical steps
SEED    = 42
MIN_N   = 30         # skip datasets with fewer samples
...
HEADER = [
    "dataset", "source", "n", "K", "dim",
    "base_members_nmi", "base_members_ari",
    "kmeans_nmi",       "kmeans_ari",
    "ward_nmi",         "ward_ari",
    "hier_base_nmi",    "hier_base_ari",
    "hier_wtd_nmi",     "hier_wtd_ari",
    "sdgca_nmi",        "sdgca_ari",
    "sdgca_mod_nmi",    "sdgca_mod_ari",
    "best_simple",      "best_consensus",
    "consensus_wins",   "consensus_gain_nmi",
    "elapsed_sec",
]
...
def collect_datasets() -> list[tuple[Path, str]]:
    """Return (path, source_label) for all usable datasets."""
...
def load_dataset(path: Path, source: str) -> tuple[np.ndarray | None, np.ndarray, np.ndarray | None]:
    """Return (X_or_None, gt, members_or_None). gt is 1-indexed int64."""
    if source == "real_npz":
        ...
        gt = gt - gt.min() + 1   # normalize to 1-indexed
        ...
        if X_raw is not None:
            ...
            X = None   # .mat without feature matrix — skip KMeans/Ward
        return X, gt, members


def base_member_metrics(members: np.ndarray, gt: np.ndarray) -> tuple[float, float]:
    """Mean NMI and ARI of ensemble partitions vs ground truth."""
    nmis, aris = [], []
...
def kmeans_metrics(X: np.ndarray, gt: np.ndarray, k: int,
                   n_runs: int = 5, seed: int = 42) -> tuple[float, float]:
...
def main() -> None:
    ...
    # AlgorithmRegistry.all() returns a list; use .get() for individual lookup
    ...
    for idx, (p, source) in enumerate(remaining):
        ...
        try:
            ...
            # -- simple baselines (only when X is available) -------------------
            if X is not None:
            ...
            cons_nmis = {k2: v[0] for k2, v in algo_results.items()
                         if not np.isnan(v[0])}
            if cons_nmis:
            ...
            consensus_wins = (not np.isnan(best_cons) and
                              not np.isnan(best_simple) and
                              best_cons > best_simple)
            ...
            row = {
                ...
                "K":               k,
                "dim":             dim,
                "base_members_nmi": round(bm_nmi, 5),
                "base_members_ari": round(bm_ari, 5),
                "kmeans_nmi":      round(km_nmi, 5),
                "kmeans_ari":      round(km_ari, 5),
                "ward_nmi":        round(wd_nmi, 5),
                "ward_ari":        round(wd_ari, 5),
                "hier_base_nmi":   round(hr_b[0], 5),
                "hier_base_ari":   round(hr_b[1], 5),
                "hier_wtd_nmi":    round(hr_w[0], 5),
                "hier_wtd_ari":    round(hr_w[1], 5),
                "sdgca_nmi":       round(sg[0], 5),
                "sdgca_ari":       round(sg[1], 5),
                "sdgca_mod_nmi":   round(sg_m[0], 5),
                "sdgca_mod_ari":   round(sg_m[1], 5),
                "best_simple":     round(best_simple, 5),
                "best_consensus":  round(best_cons, 5),
                "consensus_wins":  int(consensus_wins),
                "consensus_gain_nmi": round(gain, 5),
            ...
            print(f"  -> simple={best_simple:.4f}  consensus={best_cons:.4f}  [{marker}]",
                  flush=True)
    ...
    cols = [
        ("Base members",     "base_members_nmi"),
        ("KMeans",           "kmeans_nmi"),
        ("Ward hierarch.",   "ward_nmi"),
        ("Hier. baseline",   "hier_base_nmi"),
        ("Hier. weighted",   "hier_wtd_nmi"),
        ("SDGCA",            "sdgca_nmi"),
        ("SDGCA modified",   "sdgca_mod_nmi"),
    ]
    ...
    print()
    print(f"Average NMI gain (best_consensus - best_simple): "
          f"{avg('consensus_gain_nmi'):+.4f}")
