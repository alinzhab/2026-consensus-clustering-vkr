...
from statistical_tests_numpy import bootstrap_mean_ci, format_friedman_summary, format_nemenyi_summary, format_wilcoxon_summary, friedman_test, nemenyi_post_hoc, wilcoxon_holm

def load_combined() -> pd.DataFrame:
    parts = []
    for name in ['full_benchmark.tsv', 'analysis_full_suite.tsv', 'hierarchical_linkage_full_suite.tsv']:
        p = ROOT / 'results' / name
        if p.exists():
...
def best_per_dataset(df: pd.DataFrame, metric: str) -> pd.DataFrame:
    return df.sort_values(metric, ascending=False).drop_duplicates(['dataset', 'algorithm']).pivot(index='dataset', columns='algorithm', values=metric).dropna(axis=0, how='any')

def section_friedman_4_algorithms(df: pd.DataFrame, metric: str) -> str:
    pivot = best_per_dataset(df, metric)
    cols = ['hierarchical_baseline', 'hierarchical_weighted', 'sdgca', 'sdgca_modified']
    ...
    if fr.reject_h0(0.05) and 2 <= fr.n_algorithms <= 10:
        ...
        if all_in_cd:
            out.append(f'**Все четыре алгоритма попадают внутрь критического расстояния CD ≈ {cd:.2f} от лучшего среднего ранга. На имеющихся данных невозможно утверждать статистически значимое превосходство одного алгоритма над другим.**')
            out.append('')
...
def section_hc_baseline_vs_weighted(df: pd.DataFrame, metric: str) -> str:
    ...
    if sub.empty:
        return ''
    pivot = sub.pivot_table(index=['dataset', 'method'], columns='algorithm', values=metric, aggfunc='mean').dropna()
    if pivot.shape[0] < 5:
        return ''
    ...
    ties = int((diff == 0).sum())
    out = ['## HC weighted vs HC baseline (по парам dataset × linkage)', '', f'Сравнений: {pivot.shape[0]}. weighted > baseline: {wins}, baseline > weighted: {losses}, равенство: {ties}.', f'Среднее улучшение Δ{metric} = {diff.mean():+.4f}, медиана Δ = {diff.median():+.4f}.', '']
    n_eff = wins + losses
    if n_eff > 0:
        ...
        out.append(f'Знаковый тест (двусторонний, без учёта величины Δ): p = {p:.4f}.')
        out.append('')
    diffs = diff.values
...
def section_bootstrap(df: pd.DataFrame) -> str:
    ...
    for alg in ['hierarchical_baseline', 'hierarchical_weighted', 'sdgca', 'sdgca_modified']:
        sub = df[df['algorithm'] == alg]['nmi_mean'].dropna().values
        if sub.size < 2:
            continue
        ci = bootstrap_mean_ci(sub, confidence=0.95, n_resamples=5000, rng=rng)
        out.append(f'| {alg} | {ci.mean:.3f} | [{ci.lower:.3f}; {ci.upper:.3f}] | {sub.size} |')
    out.append('')
    return '\n'.join(out)
