...
OUTPUT_COLUMNS = ['dataset_id', 'dataset_type', 'difficulty_level', 'split', 'fold_id', 'size_profile', 'n_samples', 'n_clusters', 'dim', 'algorithm_family', 'algorithm', 'variant', 'selection_strategy', 'qd_alpha', 'linkage', 'm', 'seed', 'NMI', 'ARI', 'F-score', 'runtime_sec', 'memory_peak_mb', 'status', 'error']
DEFAULT_SINGLE = ['kmeans', 'agglomerative_average', 'agglomerative_complete', 'agglomerative_single', 'spectral']
DEFAULT_CONSENSUS = ['hierarchical_baseline', 'hierarchical_weighted', 'sdgca', 'sdgca_modified']
...
def run_single_algorithm(algorithm: str, dataset_path: Path, seed: int) -> dict[str, float]:
    ...
    if algorithm == 'kmeans':
        ...
        labels = SpectralClustering(n_clusters=n_clusters, assign_labels='kmeans', random_state=seed, affinity='nearest_neighbors', n_neighbors=min(10, max(2, x.shape[0] - 1))).fit_predict(x_scaled) + 1
    else:
        raise ValueError(f'Unknown single algorithm: {algorithm}')
    return {'NMI': compute_nmi(labels, gt), 'ARI': compute_ari(labels, gt), 'F-score': compute_pairwise_f_score(labels, gt)}

def run_consensus_algorithm(algorithm: str, dataset_path: Path, dataset_id: str, seed: int, m: int, runs: int, linkage: str, selection_strategy: str, qd_alpha: float, diffusion_mode: str) -> dict[str, float]:
    kwargs = {'dataset_path': dataset_path, 'data_name': dataset_id, 'seed': seed, 'm': m, 'cnt_times': runs, 'method': linkage, 'selection_strategy': selection_strategy, 'qd_alpha': qd_alpha}
    if algorithm == 'hierarchical_baseline':
        ...
        result = run_sdgca_modified(**kwargs, nwca_para=0.09, eta=0.75, theta=0.65, diffusion_time=diffusion_time)
    else:
        raise ValueError(f'Unknown consensus algorithm: {algorithm}')
    return {'NMI': float(result['nmi_mean']), 'ARI': float(result['ari_mean']), 'F-score': float(result['f_mean'])}
...
def row_key(row: dict[str, Any]) -> tuple[str, ...]:
    return (str(row.get('dataset_id', '')), str(row.get('algorithm_family', '')), str(row.get('algorithm', '')), str(row.get('variant', '')), str(row.get('selection_strategy', '')), str(row.get('qd_alpha', '')), str(row.get('linkage', '')), str(row.get('m', '')), str(row.get('seed', '')))
        ...
        for algorithm in algorithms:
            if algorithm in DEFAULT_SINGLE:
                if algorithm == 'spectral' and int(dataset_row['n_samples']) > 900:
                    continue
                jobs.append({'algorithm_family': 'single', 'algorithm': algorithm, 'variant': 'default', 'selection_strategy': '', 'qd_alpha': '', 'linkage': algorithm.replace('agglomerative_', '') if algorithm.startswith('agglomerative_') else '', 'm': '', 'seed': seed})
                        ...
                        for selection_strategy in args.selection_strategies:
                            ...
                            for qd_alpha in qd_values:
                                ...
                                if algorithm == 'sdgca_modified' and args.diffusion_mode == 'adaptive':
                                    variant += '_adaptive_t'
                                jobs.append({'algorithm_family': 'consensus', 'algorithm': algorithm, 'variant': variant, 'selection_strategy': selection_strategy, 'qd_alpha': qd_alpha, 'linkage': linkage, 'm': m, 'seed': seed})
            else:
...
def run_job(dataset_row: dict[str, str], job: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    ...
    row = {**{key: dataset_row.get(key, '') for key in OUTPUT_COLUMNS if key in dataset_row}, **job, 'NMI': '', 'ARI': '', 'F-score': '', 'runtime_sec': '', 'memory_peak_mb': '', 'status': 'ok', 'error': ''}
    try:
        if job['algorithm_family'] == 'single':
            metrics, runtime, peak = measure(lambda: run_single_algorithm(job['algorithm'], dataset_path, int(job['seed'])))
        else:
            metrics, runtime, peak = measure(lambda: run_consensus_algorithm(job['algorithm'], dataset_path, dataset_row['dataset_id'], int(job['seed']), int(job['m']), args.runs, job['linkage'], job['selection_strategy'], float(job['qd_alpha']) if job['qd_alpha'] != '' else 0.5, args.diffusion_mode))
...
def main() -> None:
    ...
    for dataset_row in manifest_rows:
        for job in build_jobs(dataset_row, args):
            preview = {'dataset_id': dataset_row['dataset_id'], **job}
            if row_key(preview) in done:
                skipped += 1
                continue
            row = run_job(dataset_row, job, args)
            append_tsv_row(args.output, row, OUTPUT_COLUMNS)
            written += 1
            print(f"[{written}/{total_jobs}] {row['dataset_id']} {row['algorithm']} {row['variant']} {row['status']} NMI={row['NMI']}")
