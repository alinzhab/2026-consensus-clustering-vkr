"""Run the stratified QD-selection experiment for the thesis.

The design is intentionally explicit:
- primary comparison: random vs QD-selection;
- algorithms: HC baseline, HC weighted, SDGCA, SDGCA modified;
- dataset strata: compact, overlapping, imbalanced, high-dimensional,
  elongated, density-varied, mixed-complex, and several real datasets;
- primary linkage: average by default, with optional complete/single/ward.

Outputs are saved as timestamped TSV/MD files and also mirrored to
results/designed_qd_experiment_latest.tsv for the web analytics page.
"""
...
DESIGN_DATASETS = [
    ...
    ("design_compact_easy_8k", ".npz"),
    ("design_overlap_moderate", ".npz"),
    ("design_overlap_oblong", ".npz"),
    ("design_imbalanced_6x", ".npz"),
    ("design_imbalanced_oblong", ".npz"),
    ("design_highdim_20d", ".npz"),
    ("design_highdim_40d", ".npz"),
    ("design_elongated_2d", ".npz"),
    ("design_elongated_density", ".npz"),
    ("design_density_varied_low_noise", ".npz"),
    ("design_density_varied_noisy", ".npz"),
    ("design_mixed_complex_6d", ".npz"),
    ("design_mixed_complex_branchy", ".npz"),
]
...
def run_one(
    ...
    return {
        ...
        "algorithm": algorithm,
        "selection_strategy": selection_strategy,
        "qd_alpha": qd_alpha if selection_strategy == "qd" else "",
        "linkage": linkage,
        "m": m,
        "runs": runs,
        "seed": seed,
...
def write_report(path: Path, rows: list[dict], args, tsv_path: Path) -> None:
    ...
    lines = [
        "# Designed QD-selection experiment",
        "",
        ...
        "## Experimental logic",
        "",
        "The experiment tests a concrete question for the thesis: whether the way we choose the base clusterings changes the quality of hierarchical consensus clustering.",
        "",
        "- `random`: the baseline protocol, selecting `m` base clusterings randomly from the pool.",
        "- `qd`: quality-diversity selection, choosing a deterministic subensemble that balances consensus with diversity.",
        "",
        "The analysis is stratified by dataset type. This is important because the thesis is not trying to claim that one algorithm is universally best; it studies when each algorithm works better or worse.",
        "",
        "## Parameters",
        "",
        f"- m: `{args.m}`",
        f"- runs inside each algorithm call: `{args.runs}`",
        f"- seeds: `{args.seeds}`",
        f"- linkage methods: `{args.methods}`",
        f"- qd_alpha: `{args.qd_alpha}`",
        f"- algorithms: `{args.algos}`",
        f"- rows: `{len(rows)}`",
        "",
        "## Dataset groups and hypotheses",
        "",
        "| Dataset type | Why this type is included | Expected stress for algorithms |",
        "|---|---|---|",
        "| compact | sanity check for well separated clusters | all algorithms should be stable |",
        "| overlapping | ambiguous borders | consensus may smooth noise, but may also merge classes |",
        "| imbalanced | small clusters can be swallowed | weighted methods and SDGCA may help |",
        "| high_dimensional | distance concentration and feature noise | base clustering quality becomes unstable |",
        "| elongated | non-spherical geometry | linkage choice becomes important |",
        "| density_varied | uneven local density | co-association can overfit dense regions |",
        "| mixed_complex | several difficulties at once | QD may help by removing weak subensemble members |",
        "| real_bio | real benchmark structure | external sanity check beyond synthetic data |",
        "",
        "## Delta NMI: QD - random by algorithm",
        "",
        "| Algorithm | Mean delta NMI | QD wins | QD losses | Tie |",
        "|---|---:|---:|---:|---:|",
    ]
    ...
    lines += [
        "",
        "## Delta NMI by dataset type",
        "",
        "| Dataset type | Algorithm | Mean delta NMI | Comparisons |",
        "|---|---|---:|---:|",
    ]
    ...
    lines += [
        "",
        "## Baseline random NMI by dataset type",
        "",
        "| Dataset type | Algorithm | Mean random NMI | Rows |",
        "|---|---|---:|---:|",
    ]
    ...
    lines += [
        ...
        "Allowed conclusions:",
        "",
        "- Compare QD vs random within the same dataset, algorithm, linkage, seed, and `m`.",
        "- Compare behavior across dataset types rather than only one global average.",
        "- Discuss runtime as part of the testing system, not only quality metrics.",
        "",
        "Not allowed without additional runs:",
        "",
        "- Claiming that QD-selection is universally better.",
        "- Claiming statistical significance if the number of independent seeds/datasets is too small.",
        "- Claiming that a result transfers to very large datasets without separate scalability tests.",
        "",
        "## Thesis contribution wording",
        "",
        "This experiment strengthens the thesis because it turns the system from a simple runner into an analysis platform: the same algorithms are compared under controlled dataset types and under two subensemble selection protocols.",
    ]
