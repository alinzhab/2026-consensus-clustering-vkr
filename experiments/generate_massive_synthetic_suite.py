"""Generate a stratified synthetic suite for dataset-level validation.

The suite is intentionally stored outside the small hand-made experiments:
``datasets/massive_synthetic``.  The manifest is the source of truth for
dataset-level train/validation/test splits and 5-fold CV.
"""
...
def _make_config(
    ...
    if dataset_type == "compact":
        ...
        params = {
            ...
            "dim": dim,
            "cluster_std": float(rng.uniform(0.18, 0.35)),
            "separation": float(rng.uniform(5.5, 8.0)),
            "imbalance_ratio": float(rng.uniform(1.0, 1.4)),
            "seed": seed,
            **_common_base_params(n_clusters, size_profile, smoke),
        }
        ...
        params = {
            ...
            "n_clusters": n_clusters,
            "dim": dim,
            "cluster_std": float(rng.uniform(0.65, 1.15)),
            "separation": float(rng.uniform(1.8, 3.0)),
            "imbalance_ratio": float(rng.uniform(1.0, 1.8)),
            "seed": seed,
            **_common_base_params(n_clusters, size_profile, smoke),
        }
        ...
        params = {
            ...
            "n_clusters": n_clusters,
            "dim": dim,
            "cluster_std": float(rng.uniform(0.35, 0.75)),
            "separation": float(rng.uniform(3.2, 5.5)),
            "imbalance_ratio": float(rng.uniform(3.0, 8.0)),
            "seed": seed,
            **_common_base_params(n_clusters, size_profile, smoke),
        }
        ...
        params = {
            ...
            "n_clusters": n_clusters,
            "dim": dim,
            "cluster_std": float(rng.uniform(0.45, 0.95)),
            "separation": float(rng.uniform(2.4, 4.8)),
            "imbalance_ratio": float(rng.uniform(1.0, 2.5)),
            "seed": seed,
            **_common_base_params(n_clusters, size_profile, smoke),
        }
        ...
        params = {
            ...
            "radius_ref": 1.0,
            "radius_maxmin": float(rng.uniform(1.5, 3.0)),
        ...
        params = {
            ...
            "dim": dim,
            "clunum": n_clusters,
            "core_num": int(rng.integers(max(n_clusters * 8, 30), max(n_clusters * 22, 70))),
            "data_num": n_samples,
            "seed": seed,
            "domain_size": 20.0,
            "radius": float(rng.uniform(0.025, 0.055)),
            "step": float(rng.uniform(0.04, 0.075)),
            "noise_ratio": float(rng.uniform(0.04, 0.14)),
            "density_factors": factors,
            "momentum": float(rng.uniform(0.0, 0.45)),
            "branch": float(rng.uniform(0.0, 0.08)),
            "star": float(rng.uniform(0.0, 0.20)),
        ...
        params = {
            ...
            "aspect_maxmin": float(rng.uniform(4.0, 8.0)),
            "radius_ref": 1.0,
            "radius_maxmin": float(rng.uniform(2.0, 4.0)),
...
def _assign_splits(rows: list[dict[str, Any]], seed: int, folds: int) -> None:
    ...
    for indices in by_type.values():
        ...
        n = len(shuffled)
        if n >= 3:
            train_count = max(1, int(np.floor(0.60 * n)))
            validation_count = max(1, int(np.floor(0.20 * n)))
        ...
        rows.append(
            {
                ...
                "split": "",
                "fold_id": "",
                "size_profile": meta["size_profile"],
                "allow_sdgca": str(bool(allow_sdgca)).lower(),
                "base_clusterings": int(params["base_clusterings"]),
                "base_k_min": int(params["base_k_min"]),
                "base_k_max": int(params["base_k_max"]),
