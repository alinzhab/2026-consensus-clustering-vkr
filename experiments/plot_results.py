...
ALG_ORDER = ['hierarchical_baseline', 'hierarchical_weighted', 'sdgca', 'sdgca_modified']
ALG_LABEL = {'hierarchical_baseline': 'HC base', 'hierarchical_weighted': 'HC weighted', 'sdgca': 'SDGCA', 'sdgca_modified': 'SDGCA mod'}
...
plt.rcParams.update({'figure.dpi': 110, 'savefig.dpi': 160, 'font.size': 10, 'axes.spines.top': False, 'axes.spines.right': False})
...
def plot_nmi_by_algorithm(df: pd.DataFrame) -> None:
    ...
    for ax, metric in zip(axes, METRIC_ORDER):
        data = [df.loc[df['algorithm'] == alg, metric].dropna().values for alg in ALG_ORDER]
        ax.boxplot(data, labels=[ALG_LABEL[a] for a in ALG_ORDER], showmeans=True, meanprops={'marker': 'D', 'markerfacecolor': 'white', 'markersize': 5})
        ax.set_title(METRIC_LABEL[metric])
        ax.set_ylim(-0.05, 1.05)
        ax.grid(axis='y', alpha=0.3)
        ax.tick_params(axis='x', rotation=20)
    axes[0].set_ylabel('Метрика качества')
    fig.suptitle('Распределение метрик по алгоритмам (по датасетам × linkage)', fontsize=12)
    _save(fig, '01_metrics_by_algorithm.png')

def plot_heatmap_nmi(df: pd.DataFrame) -> None:
    pivot = df.groupby(['algorithm', 'method'])['nmi_mean'].mean().unstack('method').reindex(index=ALG_ORDER, columns=METHOD_ORDER)
    ...
    for i in range(pivot.shape[0]):
        for j in range(pivot.shape[1]):
            ...
            if not np.isnan(value):
                ax.text(j, i, f'{value:.2f}', ha='center', va='center', color='white' if value < 0.5 else 'black', fontsize=10)
...
def plot_per_dataset_grouped(df: pd.DataFrame, metric: str='nmi_mean') -> None:
    best = df.sort_values(metric, ascending=False).drop_duplicates(['dataset', 'algorithm'])
    ...
    ax.set_xticks(x)
    ax.set_xticklabels([d.replace('analysis_', '').replace('_hard', '') for d in datasets], rotation=35, ha='right')
    ax.set_ylabel(METRIC_LABEL[metric])
    ax.set_ylim(0, 1.05)
    ax.grid(axis='y', alpha=0.3)
    ax.set_title(f'Лучший {METRIC_LABEL[metric]} по каждому датасету (по выбору linkage)')
...
def plot_average_ranks(df: pd.DataFrame, metric: str='nmi_mean') -> None:
    ...
    bars = ax.barh(np.arange(k), avg_ranks[order], color='#3b7dd8')
    ...
    for bar, value in zip(bars, avg_ranks[order]):
        ax.text(value + 0.05, bar.get_y() + bar.get_height() / 2, f'{value:.2f}', va='center', fontsize=10)
    ax.set_title(f'Средний ранг по {METRIC_LABEL[metric]} ({n} датасетов, CD$_{{0.05}}$ = {cd:.2f})')
    ax.grid(axis='x', alpha=0.3)
    best_rank = avg_ranks[order][0]
    ax.axvspan(best_rank, best_rank + cd, alpha=0.12, color='red')
    ax.text(best_rank + cd / 2, -0.6, 'зона неотличимости от лидера (CD)', fontsize=8, color='darkred', ha='center')
    _save(fig, f'04_average_ranks_{metric}.png')
DATASET_SIZES = {'Aggregation': 788, 'BBC': 2225, 'Ecoli': 336, 'GLIOMA': 50, 'Lung': 203, 'orlraws10P': 100, 'custom_densired_dataset': 2000, 'densired_compact_hard': 3000, 'densired_mix_hard': 3500, 'densired_stretched_hard': 3500, 'repliclust_heterogeneous_hard': 3200, 'repliclust_highdim_hard': 4000, 'repliclust_oblong_overlap': 2500, 'analysis_densired_compact': 3200, 'analysis_densired_stretched': 3800, 'analysis_highdim': 3500, 'analysis_imbalanced': 3500, 'analysis_repliclust_heterogeneous': 4000, 'analysis_repliclust_oblong': 3000, 'analysis_simple_overlap': 3500, 'analysis_simple_separated': 3000}
...
def plot_complexity_scatter(df: pd.DataFrame) -> None:
    ...
    enriched.loc[fill_mask, 'n_objects'] = enriched.loc[fill_mask, 'dataset'].map(DATASET_SIZES)
    ...
    colors = dict(zip(ALG_ORDER, ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728']))
    groups = {'HC': ['hierarchical_baseline', 'hierarchical_weighted'], 'SDGCA': ['sdgca', 'sdgca_modified']}
    ...
    for group_name, algs in groups.items():
        ...
        for alg in algs:
            ...
            if s.empty:
                continue
            ax.scatter(s['n_objects'], s['seconds'], label=ALG_LABEL[alg], alpha=0.65, color=colors[alg], s=28, edgecolor='black', linewidth=0.4)
        if len(group_data) >= 4 and group_data['n_objects'].nunique() >= 3:
            ...
            ax.plot(x_line, a_coef * x_line ** b, ls, color='black', alpha=0.5)
...
def plot_uncertainty(df: pd.DataFrame) -> None:
    ...
    ax.bar(x, means.values, yerr=stds.values, capsize=6, color='#7c3aed', alpha=0.85)
    ax.set_xticks(x)
