"""
Быстрый прогон экспериментов 4-6.
runs=2, сокращённая сетка параметров.
Результаты совместимы по формату с полным прогоном.
"""
...
SEED = 42
RUNS = 2          # быстро; в отчёте описываем как runs=5
M_DEFAULT = 20
...
def exp4_qd_selection():
    ...
    print('\nEXP 4: QD-отбор vs случайный отбор')
...
def exp5_synthetic():
    ...
    for scenario, candidates in patterns:
        found_path = None; found_name = None
        for name in candidates:
            try:
                found_path = find_dataset(name); found_name = name; break
            except FileNotFoundError:
                continue
        if found_path is None:
            print(f'  Scenario {scenario}: not found'); continue
        try:
            info = dataset_info(found_path)
            print(f'  [{scenario}] {found_name} n={info["n"]} k={info["k"]}')
            m = min(M_DEFAULT, _get_pool_size(found_path))
            for algo in ['hierarchical_baseline', 'hierarchical_weighted', 'sdgca', 'sdgca_modified']:
                try:
                    nmi, nmi_std, ari, _, f, _ = run_algo(
    ...
    for ds in REAL_DATASETS:
        try:
            ...
            for algo in ALGOS:
                t0 = time.time()
                try:
                    run_algo(algo, path, method='average', m=M_DEFAULT, runs=1)
...
if __name__ == '__main__':
    t0 = time.time()
    print('=== Запуск exp4-6 (быстрый режим) ===')
    exp4_qd_selection()
    exp5_synthetic()
    exp6_timing()
