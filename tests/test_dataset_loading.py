...
from app import _load_json_arrays, _load_table_arrays, analyze_dataset_structure, build_consensus_ready_dataset, recommend_params, summarize_dataset

def _csv_with_header(tmp_path: Path) -> Path:
    p = tmp_path / 'sample.csv'
    p.write_text('f1,f2,f3,gt\n1.0,2.0,3.0,0\n4.0,5.0,6.0,1\n7.0,8.0,9.0,0\n10.0,11.0,12.0,1\n', encoding='utf-8')
    return p
...
def _json_full(tmp_path: Path, n: int=30) -> Path:
    ...
    p.write_text(json.dumps({'X': x, 'gt': gt, 'members': members, 'meta': {'note': 'test'}}), encoding='utf-8')
    return p
...
def test_analyze_has_members_flag(tmp_path):
    ...
    p = tmp_path / 'c.npz'
    np.savez(p, X=rng.normal(size=(40, 2)), gt=np.repeat(np.arange(2), 20), members=rng.integers(1, 3, size=(40, 10)).astype(np.int64))
    info = analyze_dataset_structure(p)
...
def test_recommend_unknown_classes():
    ...
    assert rec['k_min'] == 2
    assert rec['k_max'] == 10
    assert any(('неизвестно' in w.lower() or 'отсутствует' in w.lower() or 'gt' in w.lower() for w in rec['warnings']))

def test_build_from_x_gt_creates_members(tmp_path):
    p = _npz_x_gt_only(tmp_path)
    ready_path, members = build_consensus_ready_dataset(p, n_clusterings=10, k_min=2, k_max=4, strategy='kmeans')
