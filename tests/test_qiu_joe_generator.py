...
def test_qiu_joe_generator_returns_consensus_ready_arrays():
    x, gt, members, meta = generate_qiu_joe_style_dataset(name='qiu_smoke', n_samples=60, n_clusters=3, dim=2, overlap_level='medium', separation=1.0, shape_ratio=2.0, volume_mean=1.0, imbalance_ratio=1.5, orientation='axis_aligned', noise_ratio=0.05, seed=123, base_clusterings=4, base_k_min=2, base_k_max=4, base_strategy='kmeans')
    assert x.shape == (60, 2)
    assert gt.shape == (60,)
...
def test_qiu_joe_save_dataset_writes_project_format(tmp_path):
    x, gt, members, meta = generate_qiu_joe_style_dataset(name='qiu_save_smoke', n_samples=40, n_clusters=2, dim=2, overlap_level='low', shape_ratio=1.5, imbalance_ratio=1.0, orientation='axis_aligned', seed=7, base_clusterings=3, base_k_min=2, base_k_max=3, base_strategy='kmeans')
...
@pytest.mark.parametrize('kwargs', [{'overlap_level': 'bad'}, {'orientation': 'bad'}, {'n_clusters': 1}, {'n_samples': 5, 'n_clusters': 3}, {'shape_ratio': 0.5}, {'volume_mean': 0.0}, {'noise_ratio': 0.8}])
def test_qiu_joe_generator_rejects_invalid_parameters(kwargs):
    params = {'n_samples': 40, 'n_clusters': 2, 'dim': 2, 'base_clusterings': 2, 'base_strategy': 'kmeans'}
