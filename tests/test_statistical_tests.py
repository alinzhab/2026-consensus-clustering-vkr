...
import numpy as np
import pytest
from statistical_tests import bootstrap_mean_ci, friedman_test, nemenyi_post_hoc, wilcoxon_holm
...
def test_friedman_detects_clear_winner():
    ...
    assert res.average_ranks.argmin() == 0
...
def test_friedman_rejects_invalid_inputs():
    with pytest.raises(ValueError):
        friedman_test(np.array([[1.0, 2.0]]), ['A', 'B'])
    with pytest.raises(ValueError):
        friedman_test(np.array([[np.nan, 0.5], [0.6, 0.7]]), ['A', 'B'])
