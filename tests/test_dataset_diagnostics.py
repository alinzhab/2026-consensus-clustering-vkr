"""Тесты модуля dataset_diagnostics.

Проверяем:
  Без gt — все базовые, PCA, density, Hopkins метрики присутствуют
           и лежат в ожидаемых диапазонах.
  С gt   — метрики классов, разделимость, перекрытие, центроиды, форма.
  Граничные случаи — маленький n, d=1, единственный класс, NaN/Inf в X.
  recommend_from_diagnostics — правила m, k_min/k_max, strategy,
           diffusion_time, sharpen, lambda_/eta/theta реагируют правильно.
"""
...
def well_separated():
    """3 хорошо разделённых гауссовых кластера, 150 объектов, 2 признака."""
...
@pytest.fixture(scope="module")
def overlapping():
    """3 сильно перекрывающихся кластера."""
...
@pytest.fixture(scope="module")
def high_dim():
    """Высокая размерность: 200 объектов, 80 признаков, 4 класса."""
...
@pytest.fixture(scope="module")
def imbalanced():
    """Сильный дисбаланс: классы размерами 150, 20, 10."""
    rng = np.random.default_rng(3)
    X = np.vstack([
        rng.normal([0, 0], 0.5, size=(150, 2)),
        rng.normal([6, 0], 0.5, size=(20,  2)),
        rng.normal([3, 6], 0.5, size=(10,  2)),
    ])
...
def test_basic_fields_present(well_separated):
    ...
    for key in (
        "n_objects", "n_features", "dimensionality_ratio",
        "missing_ratio", "infinite_ratio", "constant_feature_ratio",
        "feature_variance_mean", "feature_variance_std",
        "feature_variance_min", "feature_variance_max",
    ):
...
def test_pca_fields_present(well_separated):
    ...
    diag = compute_diagnostics(X)
    for key in (
        "explained_variance_2d", "explained_variance_10d",
        "effective_dimension_90", "effective_dimension_95",
        "participation_ratio",
    ):
...
def test_pca_high_dim(high_dim):
    X, _ = high_dim
    diag = compute_diagnostics(X)
    # В 80 измерениях 2D объясняет мало
    assert diag["explained_variance_2d"] < 0.50
...
def test_density_variation_higher_for_overlapping(well_separated, overlapping):
    """Перекрывающиеся кластеры = более однородная плотность."""
    Xw, _ = well_separated
    Xo, _ = overlapping
    diag_w = compute_diagnostics(Xw)
    diag_o = compute_diagnostics(Xo)
    # Разделённые кластеры: внутри кластера плотность другая, чем между —
    # density_variation должна быть выше
...
def test_hopkins_higher_for_clustered_vs_random():
    """Хорошо сгруппированные данные должны иметь Hopkins > 0.6."""
...
def test_class_stats_fields(well_separated):
    ...
    for key in ("n_classes", "class_size_min", "class_size_max",
                "imbalance_ratio", "small_class_ratio", "class_entropy"):
        assert key in diag
...
def test_overlapping_high_overlap():
    """Жёсткое перекрытие: 3 кластера с одинаковым центром, разные метки → overlap_ratio > 0."""
    rng = np.random.default_rng(99)
    # 3 сильно перекрывающихся кластера — центры совпадают
    X = np.vstack([
        rng.normal([0.0, 0.0], 1.5, size=(30, 2)),
        rng.normal([0.5, 0.5], 1.5, size=(30, 2)),
        rng.normal([1.0, 0.0], 1.5, size=(30, 2)),
    ])
    gt = np.repeat(np.arange(3), 30)
    diag = compute_diagnostics(X, gt=gt)
    # При сильном перекрытии silhouette отрицательный или очень низкий
    sil = diag.get("silhouette_score")
    if sil is not None:
        assert sil < 0.4, "Strongly overlapping clusters should have low silhouette"
    # overlap_ratio ≥ 0 (может быть 0 если все nearest-friends ближе, это допустимо)
    assert 0.0 <= diag["overlap_ratio"] <= 1.0
...
def test_elongated_clusters_higher_elongation():
    """Вытянутые кластеры должны иметь elongation_mean значительно > 1."""
    rng = np.random.default_rng(7)
    # Вытянутые вдоль оси X
    X_elong = np.vstack([
        rng.normal([0, 0], [5.0, 0.3], size=(60, 2)),
        rng.normal([0, 8], [5.0, 0.3], size=(60, 2)),
    ])
    gt_elong = np.repeat([0, 1], 60)
    # Изотропные
    X_round = np.vstack([
        rng.normal([0, 0], 1.0, size=(60, 2)),
        rng.normal([0, 8], 1.0, size=(60, 2)),
    ])
...
def test_single_class_gt(well_separated):
    ...
    # Не должно падать; разделимость и перекрытие с ошибкой — допустимо
...
def test_very_small_n():
    ...
    # Hopkins может вернуть None при n < 15
    assert "hopkins" in diag
...
def test_recommend_k_range_from_n_classes(well_separated):
    ...
    diag = compute_diagnostics(X, gt=gt)
    rec  = recommend_from_diagnostics(diag)
    # n_classes=3 → k_min in [1,3], k_max in [3,10]
    assert rec["k_min"] >= 1
...
def test_recommend_imbalanced_increases_m(imbalanced):
    ...
    diag = compute_diagnostics(X, gt=gt)
    rec  = recommend_from_diagnostics(diag)
    # При imbalance_ratio > 5 должно добавляться +10 к m
...
def test_recommend_no_gt():
    """Рекомендации без gt не должны падать."""
