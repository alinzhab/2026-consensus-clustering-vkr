...
def prepare_rows(rows):
    prepared = []
    for row in rows:
        prepared.append({'dataset': row['dataset'], 'type': row['type'], 'objects': int(row['objects']) if row.get('objects') else None, 'features': int(row['features']) if row.get('features') else None, 'classes': int(row['classes']) if row.get('classes') else None, 'imbalance_ratio': parse_float(row.get('imbalance_ratio', '')), 'sdgca_nmi': parse_float(row.get('sdgca_nmi', '')), 'sdgca_nmi_std': parse_float(row.get('sdgca_nmi_std', '')), 'sdgca_ari': parse_float(row.get('sdgca_ari', '')), 'sdgca_ari_std': parse_float(row.get('sdgca_ari_std', '')), 'sdgca_f': parse_float(row.get('sdgca_f', '')), 'sdgca_f_std': parse_float(row.get('sdgca_f_std', '')), 'mod_nmi': parse_float(row.get('mod_nmi', '')), 'mod_nmi_std': parse_float(row.get('mod_nmi_std', '')), 'mod_ari': parse_float(row.get('mod_ari', '')), 'mod_ari_std': parse_float(row.get('mod_ari_std', '')), 'mod_f': parse_float(row.get('mod_f', '')), 'mod_f_std': parse_float(row.get('mod_f_std', '')), 'delta_nmi': parse_float(row.get('delta_nmi', '')), 'delta_ari': parse_float(row.get('delta_ari', '')), 'delta_f': parse_float(row.get('delta_f', '')), 'winner': row.get('winner', '')})
    return prepared

def rows_with_complete_metrics(rows):
    return [row for row in rows if row['delta_nmi'] is not None and row['delta_ari'] is not None and (row['delta_f'] is not None)]
...
def save_delta_heatmap(rows):
    ...
    vmax = max(abs(matrix.min()), abs(matrix.max()), 1e-06)
...
def save_dumbbell_panels(rows):
    metric_specs = [('nmi', 'NMI'), ('ari', 'ARI'), ('f', 'F-score')]
...
def save_imbalance_scatter(rows):
    ...
    for row in rows:
        ax.scatter(row['imbalance_ratio'], row['delta_nmi'], color=colors.get(row['type'], '#333333'), s=70, alpha=0.85)
    ...
    handles = [plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=colors['real'], markersize=8, label='Реальные'), plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=colors['synthetic'], markersize=8, label='Синтетические')]
...
def save_std_errorbars_real(rows):
    rows = [row for row in rows_grouped(rows, 'real') if row['sdgca_nmi_std'] is not None and row['mod_nmi_std'] is not None and (row['sdgca_ari_std'] is not None) and (row['mod_ari_std'] is not None) and (row['sdgca_f_std'] is not None) and (row['mod_f_std'] is not None)]
...
def save_objects_scatter(rows):
    ...
    for row in rows:
        ax.scatter(row['objects'], row['delta_f'], color=colors.get(row['type'], '#333333'), s=70, alpha=0.85)
...
def save_profile_bubble(rows):
    ...
    for row in rows:
        ...
        ax.scatter(row['imbalance_ratio'], row['delta_ari'], s=size, c=color_map.get(row['winner'], '#1f77b4'), marker=marker_map.get(row['type'], 'o'), alpha=0.7, edgecolors='black', linewidths=0.6)
    ...
    type_handles = [plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='lightgray', markeredgecolor='black', label='Реальные', markersize=8), plt.Line2D([0], [0], marker='s', color='w', markerfacecolor='lightgray', markeredgecolor='black', label='Синтетические', markersize=8)]
    winner_handles = [plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=color_map['modified'], label='Лучше modified', markersize=8), plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=color_map['baseline'], label='Лучше baseline', markersize=8), plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=color_map['tie'], label='Ничья', markersize=8)]
