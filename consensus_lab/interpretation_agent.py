...
def interpret_dataset(analysis: dict[str, Any], recommendations: dict[str, Any]) -> dict[str, Any]:
    ...
    findings: list[str] = []
    warnings: list[str] = []
    findings.append(f"Dataset size: {n or 'unknown'} objects, {d or 'unknown'} features" + (f', {k} classes by gt.' if k is not None else '.'))
    if has_members:
    ...
    if has_gt:
        findings.append('gt is available and must be used only for evaluation/diagnostics, not for building clusters.')
    else:
        warnings.append('gt is absent: recommendations are based on internal structure diagnostics, not NMI/ARI/F-score.')
    dim_level = _level(analysis.get('dimensionality_ratio'), 0.1, 0.5)
    density_level = _level(density_variation, 0.3, 0.6)
    overlap_level = _level(overlap_ratio, 0.15, 0.3)
    ...
    summary = f"The agent recommends starting with {primary}, linkage={primary_params.get('method', 'average')}, m={recommendations.get('m')}, selection strategy from the experiment settings."
    evidence = {'silhouette_score': _fmt(silhouette), 'density_variation': _fmt(density_variation), 'overlap_ratio': _fmt(overlap_ratio), 'imbalance_ratio': _fmt(imbalance_ratio), 'elongation_max': _fmt(elongation_max), 'explained_variance_2d': _fmt(exp_2d), 'outlier_ratio': _fmt(outlier_ratio)}
    return {'title': 'AI interpretation agent', 'summary': summary, 'findings': findings, 'warnings': warnings, 'recommended_algorithm': primary, 'recommended_parameters': {'m': recommendations.get('m'), 'k_min': recommendations.get('k_min'), 'k_max': recommendations.get('k_max'), 'strategy': recommendations.get('strategy'), 'preprocessing': recommendations.get('preprocessing'), **primary_params}, 'evidence': evidence, 'protocol_note': 'This is an interpretation layer. It explains diagnostics and recommendations; it does not participate in clustering and does not tune on the test split.'}
