...
FULL_GRID = {'nwca_para': [0.02, 0.04, 0.06, 0.07, 0.08, 0.09, 0.1, 0.15, 0.18, 0.3, 0.5, 0.75, 0.95], 'eta': [0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95, 1.01], 'theta': [0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95, 1.01]}
...
def _warm_start_points(n: int, K: int, d: int, top_k: int=5) -> list[dict]:
    ...
    def dist(row: dict) -> float:
        ...
        return dn + 2 * dk + 0.5 * dd
    ranked = sorted(PAPER_GRID, key=dist)
    return [{'nwca_para': r['lam'], 'eta': r['eta'], 'theta': r['theta']} for r in ranked[:top_k]]

def _compute_metrics(X: np.ndarray | None, labels: np.ndarray, base_cls: np.ndarray) -> dict[str, float]:
...
def _run_one(members: np.ndarray, gt: np.ndarray, X: np.ndarray | None, nwca_para: float, eta: float, theta: float, m: int, seed: int, method: str) -> dict[str, float]:
    import sys
    sys.path.insert(0, str(Path(__file__).parent))
    from sdgca import get_all_segs, compute_nwca, compute_neci, compute_d, compute_s, compute_w, optimize_sdgca
...
def _objective(members: np.ndarray, gt: np.ndarray, X: np.ndarray | None, nwca_para: float, eta: float, theta: float, m: int, seed: int, method: str, n_avg: int) -> float:
    scores: list[float] = []
    for run in range(n_avg):
        try:
            r = _run_one(members, gt, X, nwca_para, eta, theta, m, seed + run * 97, method)
            sil_n = (r['silhouette'] + 1) / 2
            db_n = 1.0 / (1.0 + r['davies_bouldin'])
            agr = r['ensemble_agreement']
            composite = 0.4 * sil_n + 0.4 * agr + 0.2 * db_n
            scores.append(composite)
        except Exception:
            scores.append(0.0)
    return float(np.mean(scores)) if scores else 0.0

def tune_sdgca(dataset_path: str | Path, n_trials: int=80, m: int=20, seed: int=42, method: str='ward', n_avg: int=2, verbose: bool=True, use_gt_for_validation: bool=True) -> dict[str, Any]:
    import optuna
    ...
    def optuna_objective(trial: optuna.Trial) -> float:
        ...
        eta = trial.suggest_float('eta', 0.6, 1.05)
        theta = trial.suggest_float('theta', 0.6, 1.05)
        return _objective(members, gt, X, lam, eta, theta, m, seed, method, n_avg)
    sampler = optuna.samplers.TPESampler(seed=seed, n_startup_trials=max(10, len(warm_pts)), multivariate=True, warn_independent_sampling=False)
    study = optuna.create_study(direction='maximize', sampler=sampler)
    for pt in warm_pts:
        study.enqueue_trial(pt)
    anchors = [{'nwca_para': 0.06, 'eta': 0.75, 'theta': 0.75}, {'nwca_para': 0.06, 'eta': 1.01, 'theta': 1.01}, {'nwca_para': 0.15, 'eta': 0.7, 'theta': 0.65}]
    for a in anchors:
    ...
    result: dict[str, Any] = {'nwca_para': best['nwca_para'], 'eta': best['eta'], 'theta': best['theta'], 'score': best_score, 'n_trials': n_trials, 'dataset': Path(dataset_path).stem}
    if use_gt_for_validation:
        ...
        for run in range(5):
            try:
                r = _run_one(members, gt, X, best['nwca_para'], best['eta'], best['theta'], m, seed + run * 13, method)
...
def tune_and_compare(dataset_path: str | Path, n_trials: int=80, m: int=20, seed: int=42, method: str='ward', verbose: bool=True) -> dict[str, Any]:
    import sys
    sys.path.insert(0, str(Path(__file__).parent))
    ...
    print()
    best = tune_sdgca(dataset_path, n_trials=n_trials, m=m, seed=seed, method=method, verbose=verbose)
    print()
    print('=' * 60)
...
def grid_search_sdgca(dataset_path: str | Path, m: int=20, seed: int=42, method: str='ward', coarse: bool=True, verbose: bool=True) -> dict[str, Any]:
    import sys
    sys.path.insert(0, str(Path(__file__).parent))
    members, gt, X = _load(dataset_path)
    if coarse:
        ...
        theta_grid = [0.6, 0.7, 0.8, 0.9, 1.01]
    else:
    ...
    for lam in lam_grid:
        for eta in eta_grid:
            for theta in theta_grid:
                score = _objective(members, gt, X, lam, eta, theta, m, seed, method, n_avg=1)
                ...
                if verbose and done % 25 == 0:
                    print(f"  [{done}/{total}] best so far: lam={best_params['nwca_para']:.3f} eta={best_params['eta']:.3f} theta={best_params['theta']:.3f} score={best_score:.4f}")
    if verbose:
        print(f"\nBest: lam={best_params['nwca_para']:.3f} eta={best_params['eta']:.3f} theta={best_params['theta']:.3f} score={best_score:.4f}")
    return {'best': best_params, 'all': all_results}
if __name__ == '__main__':
    import argparse, json, sys
    sys.path.insert(0, str(Path(__file__).parent))
    parser = argparse.ArgumentParser(description='Smart hyperparameter tuning for SDGCA')
    ...
    parser.add_argument('--mode', choices=['bayesian', 'grid', 'compare'], default='bayesian', help='bayesian=TPE (default), grid=det. grid, compare=default vs tuned')
    parser.add_argument('--coarse', action='store_true', help='Coarse grid (5×5×5) instead of full 13×9×9')
    parser.add_argument('--quiet', action='store_true')
    args = parser.parse_args()
    verbose = not args.quiet
    if args.mode == 'bayesian':
        result = tune_sdgca(args.dataset, n_trials=args.trials, m=args.m, seed=args.seed, method=args.method, verbose=verbose)
        print('\nResult JSON:')
        print(json.dumps({k: v for k, v in result.items() if k != 'labels'}, indent=2))
    elif args.mode == 'grid':
        result = grid_search_sdgca(args.dataset, m=args.m, seed=args.seed, method=args.method, coarse=args.coarse, verbose=verbose)
        print('\nBest JSON:')
        print(json.dumps(result['best'], indent=2))
    elif args.mode == 'compare':
        result = tune_and_compare(args.dataset, n_trials=args.trials, m=args.m, seed=args.seed, method=args.method, verbose=verbose)
        print('\nResult JSON:')
        print(json.dumps({k: v for k, v in result.items() if k != 'labels'}, indent=2))
