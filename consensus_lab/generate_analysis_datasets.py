...
def generate(output_dir: Path, base_clusterings: int) -> None:
    ...
    simple_configs = [
        (
            "analysis_simple_separated",
            dict(n_samples=3000, n_clusters=6, dim=2, cluster_std=0.45, separation=5.0, imbalance_ratio=1.0, seed=101),
        ),
        (
            "analysis_simple_overlap",
            dict(n_samples=3500, n_clusters=6, dim=2, cluster_std=1.20, separation=2.2, imbalance_ratio=1.5, seed=102),
        ),
        (
            "analysis_highdim",
            dict(n_samples=3500, n_clusters=8, dim=20, cluster_std=0.90, separation=4.0, imbalance_ratio=2.0, seed=103),
        ),
        (
            "analysis_imbalanced",
            dict(n_samples=3500, n_clusters=8, dim=5, cluster_std=0.80, separation=3.5, imbalance_ratio=5.0, seed=104),
        ),
    ]
    for name, cfg in simple_configs:
        ...
        x, gt, members, meta = generate_simple_gaussian_dataset(
            ...
            base_k_min=2,
            base_k_max=max(8, cfg["n_clusters"] + 3),
            base_strategy="mixed",
            **cfg,
        )
    ...
    densired_configs = [
        (
            ...
            dict(
                ...
                step=0.05,
                noise_ratio=0.08,
                density_factors=[1.0, 0.6, 1.4, 0.8, 1.8, 0.5, 1.2, 0.9],
                momentum=0.0,
                branch=0.0,
                star=0.0,
