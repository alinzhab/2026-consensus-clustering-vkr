...
DATASET_GROUPS = {'Aggregation': 'real_geometric', 'BBC': 'real_text', 'Ecoli': 'real_biological', 'GLIOMA': 'real_biological', 'Lung': 'real_biological', 'orlraws10P': 'real_images', 'custom_densired_dataset': 'synthetic_density', 'densired_compact_hard': 'synthetic_density', 'densired_mix_hard': 'synthetic_density', 'densired_stretched_hard': 'synthetic_density', 'repliclust_heterogeneous_hard': 'synthetic_archetype', 'repliclust_highdim_hard': 'synthetic_archetype', 'repliclust_oblong_overlap': 'synthetic_archetype'}
...
def normalize_rows(rows: list[dict], source: str) -> list[dict]:
    ...
    for row in rows:
        ...
        if nmi is None or ari is None or f_score is None:
            continue
        normalized.append({'dataset': row['dataset'], 'group': DATASET_GROUPS.get(row['dataset'], 'other'), 'algorithm': row.get('algorithm') or row.get('version') or row.get('mode'), 'method': row.get('method', 'average') or 'average', 'nmi': nmi, 'ari': ari, 'f_score': f_score, 'source': source})
    return normalized
...
def main() -> None:
    ...
    for (dataset, algorithm), row in sorted(best_by_key(linkage_rows, ('dataset', 'algorithm'), 'nmi').items()):
        report.append(f"| {dataset} | {algorithm} | {row['method']} | {row['nmi']:.3f} | {row['ari']:.3f} | {row['f_score']:.3f} |")
    report.append('')
