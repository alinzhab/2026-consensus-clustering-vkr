...
def generate_archetype_dataset(name, n_clusters, dim, n_samples, aspect_ref, aspect_maxmin, radius_ref, radius_maxmin, min_overlap, max_overlap, imbalance_ratio, distributions, distribution_proportions, seed, base_clusterings=30, base_k_min=None, base_k_max=None, base_strategy='mixed'):
    ...
    centers = place_centers(k=n_clusters, dim=dim, axes_bank=[np.diag(a) @ r.T for a, r in zip(axis_lengths, rotations)], min_overlap=min_overlap, max_overlap=max_overlap, domain_scale=4.0 * np.max(radii) * np.sqrt(dim) * n_clusters / 4.0, rng=rng)
    ...
    members, base_info = build_base_clusterings_from_features(x, n_clusterings=base_clusterings, k_min=base_k_min, k_max=base_k_max, rng=rng, strategy=base_strategy, return_info=True)
    meta = {'name': name, 'n_clusters': n_clusters, 'dim': dim, 'n_samples': n_samples, 'aspect_ref': aspect_ref, 'aspect_maxmin': aspect_maxmin, 'radius_ref': radius_ref, 'radius_maxmin': radius_maxmin, 'min_overlap': min_overlap, 'max_overlap': max_overlap, 'imbalance_ratio': imbalance_ratio, 'distributions': distributions, 'distribution_proportions': distribution_proportions, 'seed': seed, 'base_clusterings': base_clusterings, 'base_k_min': base_k_min, 'base_k_max': base_k_max, 'base_strategy': base_strategy, 'base_info': base_info}
    return (x, gt, members, meta)

def get_presets():
    return {'repliclust_oblong_overlap': {'n_clusters': 6, 'dim': 2, 'n_samples': 2500, 'aspect_ref': 4.0, 'aspect_maxmin': 6.0, 'radius_ref': 1.0, 'radius_maxmin': 2.0, 'min_overlap': 0.08, 'max_overlap': 0.18, 'imbalance_ratio': 2.5, 'distributions': ['normal', 'student_t', 'lognormal'], 'distribution_proportions': [0.5, 0.3, 0.2], 'seed': 11}, 'repliclust_heterogeneous_hard': {'n_clusters': 8, 'dim': 5, 'n_samples': 3200, 'aspect_ref': 3.5, 'aspect_maxmin': 8.0, 'radius_ref': 1.0, 'radius_maxmin': 3.0, 'min_overlap': 0.05, 'max_overlap': 0.22, 'imbalance_ratio': 3.5, 'distributions': ['normal', 'student_t', 'exponential', 'lognormal'], 'distribution_proportions': [0.35, 0.25, 0.2, 0.2], 'seed': 17}, 'repliclust_highdim_hard': {'n_clusters': 10, 'dim': 12, 'n_samples': 4000, 'aspect_ref': 2.5, 'aspect_maxmin': 5.0, 'radius_ref': 1.0, 'radius_maxmin': 2.5, 'min_overlap': 0.04, 'max_overlap': 0.16, 'imbalance_ratio': 4.0, 'distributions': ['normal', 'student_t', 'uniform'], 'distribution_proportions': [0.5, 0.35, 0.15], 'seed': 23}}
