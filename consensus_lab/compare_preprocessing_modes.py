...
def make_datasets(seed=2026):
    ...
    return [
        _make_gaussian_dataset(
            name="balanced_same_scale",
            centers=np.array([[-4.0, -4.0], [0.0, 4.0], [4.0, -4.0]]),
            stds=np.array([[0.7, 0.7], [0.7, 0.7], [0.7, 0.7]]),
            counts=[180, 180, 180],
            rng=rng,
        ),
        _make_gaussian_dataset(
            name="feature_scale_imbalance",
            centers=np.array([[-4.0, -4.0], [0.0, 4.0], [4.0, -4.0]]),
            stds=np.array([[0.8, 0.8], [0.8, 0.8], [0.8, 0.8]]),
            counts=[180, 180, 180],
            rng=rng,
            extra_scale=[1.0, 150.0],
        ),
        _make_gaussian_dataset(
            name="with_outliers",
            centers=np.array([[-4.0, -4.0], [0.0, 4.0], [4.0, -4.0]]),
        ...
        _make_gaussian_dataset(
            name="anisotropic_imbalanced",
            centers=np.array([[-5.0, -1.0], [0.0, 3.5], [5.0, -1.0]]),
            stds=np.array([[1.8, 0.25], [0.35, 1.5], [1.1, 0.45]]),
            counts=[260, 160, 90],
            rng=rng,
        ),
    ]
...
def evaluate_members(members, gt):
    ...
    return {
        "base_nmi_mean": float(np.mean(base_nmi)),
        "base_ari_mean": float(np.mean(base_ari)),
        "consensus_nmi": float(compute_nmi(labels, gt)),
        "consensus_ari": float(compute_ari(labels, gt)),
    }
    ...
    for dataset in make_datasets():
        for mode in PREPROCESSING_MODES:
            members = build_base_clusterings(
                dataset["X"],
                n_clusterings=24,
                k_min=max(2, dataset["n_clusters"] - 1),
                k_max=dataset["n_clusters"] + 1,
                rng=19,
                strategy="mixed",
                preprocessing=mode,
...
def print_summary(rows):
    ...
    for row in rows:
        print(
            f"{row['dataset']}\t{row['preprocessing']}\t"
            f"{row['base_nmi_mean']:.4f}\t{row['base_ari_mean']:.4f}\t"
            f"{row['consensus_nmi']:.4f}\t{row['consensus_ari']:.4f}"
        )
    ...
    for mode in PREPROCESSING_MODES:
        ...
        print(
            f"{mode}: base_nmi={mean_base_nmi:.4f}, "
            f"consensus_nmi={mean_consensus_nmi:.4f}, consensus_ari={mean_consensus_ari:.4f}"
        )
