"""SDGCA: Similarity and Dissimilarity Guided Co-association Matrix.

Базовая реализация алгоритма консенсус-кластеризации SDGCA. На вход —
ансамбль базовых разбиений `members`, на выход — финальное разбиение и
метрики качества по `gt`.

Ключевые этапы:
    1. `compute_neci`     — взвешивание кластеров по нормализованной
                            энтропии относительно остальных разбиений.
    2. `compute_nwca`     — нормализованная co-association матрица
                            с учётом весов кластеров.
    3. `compute_d`        — матрица «диссимилярности» по случайному
                            блужданию на графе кластерного сходства.
    4. `optimize_sdgca`   — ADMM-оптимизация под must-link / cannot-link
                            ограничениями.
    5. `compute_w`        — финальная объединённая similarity-матрица.
    6. `get_cls_result`   — иерархическая агрегация (импортируется из
                            `hierarchical_consensus`, чтобы валидация
                            и предупреждение про `ward` были общими).

Все NMI/ARI/F-score считаются через `metrics.py`.
"""
...
def get_all_segs(base_cls: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    """Глобально перенумерованные метки + индикаторная матрица кластеров.

    Args:
        base_cls: `(n, m)` матрица меток (нумерация в каждом разбиении
            начинается с 1).

    Returns:
        Кортеж `(bcs, base_cls_segs)`:
            `bcs`           — `(n, m)` метки в общей нумерации (без
                              пересечений между разбиениями).
            `base_cls_segs` — `(K, n)` бинарная индикаторная матрица,
                              где `K` — суммарное число кластеров.
    """
    n, m = base_cls.shape
...
def compute_norm_k(bcs: np.ndarray) -> np.ndarray:
    """Нормировочные множители: число кластеров в разбиении, к которому
    относится каждый из `K` глобальных кластеров.
    """
...
def get_one_cls_entropy(part_bcs: np.ndarray, norm_k_i: float) -> float:
    """Нормированная энтропия одного кластера относительно остальных разбиений."""
    entropy = 0.0
...
def get_all_cls_entropy(bcs: np.ndarray, base_cls_segs: np.ndarray) -> np.ndarray:
    """Нормированная энтропия для всех `K` глобальных кластеров."""
...
def compute_neci(
    bcs: np.ndarray, base_cls_segs: np.ndarray, para_theta: float
) -> np.ndarray:
    """NECI: вес кластера, обратно зависящий от его энтропии.

    `para_theta` — масштаб (меньше → быстрее затухает с энтропией,
    больше → выравнивает все кластеры). В исходной статье:
    `lambda` ≈ 0.05–0.1 для большинства датасетов.
    """
    m = bcs.shape[1]
...
def compute_nwca(
    base_cls_segs: np.ndarray, neci: np.ndarray, m: int
) -> np.ndarray:
    """Нормализованная weighted co-association `(n, n)`.

    Веса кластеров `neci` входят в скалярное произведение индикаторных
    столбцов. Результат симметризован, отнормирован максимумом и
    приведён к диагонали = 1 (как принято в SDGCA).
    """
    base_cls_segs_t = base_cls_segs.T
...
def simxjac(a: np.ndarray, b: np.ndarray | None = None) -> np.ndarray:
    """Жаккаровская похожесть строк двух матриц.

    Если `b is None`, считается похожесть `a` к самой себе.
    """
    if b is None:
        b = a
    temp = a @ b.T
...
def random_walk_of_cluster(
    w: np.ndarray, k: int = 20, beta: float = 1.0
) -> np.ndarray:
    """Иерархическое случайное блуждание на графе кластерного сходства.

    Реализует схему propagation, как в исходной статье SDGCA:
    `R = sum_{i=1..k} β^i · (W~)^i (W~)^iᵀ` после row-нормализации.
    Изолированные кластеры зануляются.
    """
    n = w.shape[0]
    w = w.copy()
...
def compute_d(
    bcs: np.ndarray, base_cls_segs: np.ndarray, tau: float = 0.8
) -> np.ndarray:
    """Cannot-link матрица: усреднённая дистанция случайного блуждания
    между кластерами, обнулённая ниже порога `tau`.
    """
    n, m = bcs.shape
    d = np.zeros((n, n), dtype=np.float64)
...
def compute_s(nwca: np.ndarray, mla: np.ndarray) -> np.ndarray:
    """Must-link матрица из NWCA, приподнятая в диапазон `[0.8, 1.0]`."""
    ml = nwca.copy()
    ml[mla == 0] = 0.0
...
def optimize_sdgca(
    ...
    """ADMM-оптимизация SDGCA: ищет `S` и `D` под ограничениями.

    Возвращает пару `(S, D)`, где `S` — обновлённая similarity-матрица,
    `D` — обновлённая dissimilarity-матрица. Сходимость по фробениусовой
    норме приращений с порогом `1e-3`.
    """
    n = l_matrix.shape[0]
...
def compute_w(s: np.ndarray, d: np.ndarray, w: np.ndarray) -> np.ndarray:
    """Финальное объединение `(S, D, W)` в одну similarity-матрицу.

    Идея: где `S > D` (пара «вместе») — увеличиваем `W` в сторону 1,
    где `S < D` (пара «врозь») — уменьшаем в сторону 0.
    """
    s = np.clip(s, 0.0, 1.0)
    s = (s + s.T) / 2.0
    d = np.clip(d, 0.0, 1.0)
...
def _clamp_m(m: int, pool_size: int, algorithm_name: str) -> int:
    """Если запрошенный `m` больше пула базовых кластеризаций — усечь
    с предупреждением (молчаливое усечение скрывает реальный протокол)."""
    if m > pool_size:
        warnings.warn(
            f"{algorithm_name}: requested m={m} exceeds pool size "
            f"{pool_size}; using m={pool_size}",
            RuntimeWarning,
            stacklevel=3,
        )
        return pool_size
...
def run_sdgca(
    ...
    """Прогнать базовый SDGCA и собрать средние метрики по `cnt_times` прогонам."""
