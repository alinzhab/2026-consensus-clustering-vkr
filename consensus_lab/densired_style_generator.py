...
def is_separated_from_other_clusters(candidate: np.ndarray, cores_by_cluster: dict[int, list[np.ndarray]], cluster_id: int, epsilon: float, separation_factor: float) -> bool:
...
def grow_cluster_skeleton(dim: int, cluster_id: int, core_count: int, domain_size: float, epsilon: float, step_ratio: float, momentum: float, branch_prob: float, star_prob: float, separation_factor: float, rng: np.random.Generator, cores_by_cluster: dict[int, list[np.ndarray]], max_attempts_per_core: int=300, max_restarts: int=50) -> list[np.ndarray]:
    ...
    for _ in range(max_restarts):
        ...
        for core_idx in range(core_count):
            ...
            for _ in range(max_attempts_per_core):
                ...
                if is_separated_from_other_clusters(candidate, cores_by_cluster, cluster_id, epsilon, separation_factor):
        ...
        if len(cores) == core_count:
            ...
            if core_components(arr, epsilon) == 1:
                return cores
    raise RuntimeError(f'Could not generate cluster {cluster_id}. Try increasing domain_size or decreasing epsilon/core_count/clunum.')
...
def sample_points_around_core(center: np.ndarray, ball_radius: float, count: int, rng: np.random.Generator, point_distribution: str='uniform') -> np.ndarray:
...
def sample_noise(x: np.ndarray, count: int, epsilon: float, rng: np.random.Generator, max_batches: int=100) -> np.ndarray:
    if count <= 0:
...
def instantiate_points(cores_by_cluster: dict[int, list[np.ndarray]], data_num: int, epsilon: float, min_pts: int, density_factors: list[float], noise_ratio: float, rng: np.random.Generator, point_distribution: str='uniform') -> tuple[np.ndarray, np.ndarray]:
    ...
    for cluster_id in range(clunum):
        ...
        counts = allocate_points_per_core(total=target, n_cores=len(cores), min_pts=min_pts, rng=rng)
        radius = 0.5 * epsilon * density_factors[cluster_id]
        for core, count in zip(cores, counts):
            pts = sample_points_around_core(core, radius, int(count), rng, point_distribution=point_distribution)
            data.append(pts)
...
def validate_eps_connectivity(x: np.ndarray, gt: np.ndarray, epsilon: float) -> dict:
    result = {'epsilon': float(epsilon), 'noise_count': int(np.sum(gt == 0)), 'clusters': {}}
    for label in sorted((int(v) for v in np.unique(gt) if v > 0)):
        pts = x[gt == label]
        if len(pts) <= 1:
            result['clusters'][label] = {'n_points': int(len(pts)), 'components': 1}
            continue
        dist = cdist(pts, pts)
        adj = dist <= epsilon
        ...
        for start in range(len(pts)):
            ...
            while stack:
                ...
                visited[neighbors] = True
                stack.extend(neighbors.tolist())
        result['clusters'][label] = {'n_points': int(len(pts)), 'components': int(components)}
    return result

def generate_densired_dataset(name: str, dim: int=2, clunum: int=8, core_num: int | list[int]=120, data_num: int=3000, seed: int=19, domain_size: float=30.0, epsilon: float=0.55, step_ratio: float=0.75, min_pts: int=8, noise_ratio: float=0.05, density_factors: float | list[float]=1.0, momentum: float | list[float]=0.5, branch: float | list[float]=0.05, star: float | list[float]=0.0, separation_factor: float=1.15, normalize: bool=True, point_distribution: str='uniform', base_clusterings: int=30, base_k_min: int | None=None, base_k_max: int | None=None, base_strategy: str='mixed') -> tuple[np.ndarray, np.ndarray, np.ndarray, dict]:
    rng = np.random.default_rng(seed)
    pd = str(point_distribution).strip().lower() or 'uniform'
    if pd in {'normal'}:
        pd = 'gaussian'
    if pd not in {'uniform', 'gaussian'}:
        warnings.warn(f"Unknown point_distribution={point_distribution!r}, using 'uniform'.", RuntimeWarning, stacklevel=2)
        pd = 'uniform'
    ...
    for cluster_id in range(clunum):
        cores_by_cluster[cluster_id] = grow_cluster_skeleton(dim=dim, cluster_id=cluster_id, core_count=core_counts[cluster_id], domain_size=domain_size, epsilon=epsilon, step_ratio=step_ratio, momentum=momentum[cluster_id], branch_prob=branch[cluster_id], star_prob=star[cluster_id], separation_factor=separation_factor, rng=rng, cores_by_cluster=cores_by_cluster)
    x_raw, gt = instantiate_points(cores_by_cluster=cores_by_cluster, data_num=data_num, epsilon=epsilon, min_pts=min_pts, density_factors=density_factors, noise_ratio=noise_ratio, rng=rng, point_distribution=pd)
    ...
    for cluster_id, cores in cores_by_cluster.items():
        arr = np.vstack(cores)
        core_diagnostics[cluster_id + 1] = {'core_count': int(len(arr)), 'core_components': int(core_components(arr, epsilon))}
    meta = {'name': name, 'generator': 'densired_full_practical', 'note': "Practical DENSIRED-style implementation based on skeleton construction and hypersphere instantiation. Not a bitwise reproduction of the authors' code.", 'dim': int(dim), 'clunum': int(clunum), 'core_num': core_num, 'core_counts': core_counts, 'data_num_target': int(data_num), 'data_num_actual': int(x.shape[0]), 'seed': int(seed), 'domain_size': float(domain_size), 'epsilon_raw_space': float(epsilon), 'step_ratio': float(step_ratio), 'min_pts': int(min_pts), 'noise_ratio': float(noise_ratio), 'noise_label': 0, 'density_factors': density_factors, 'momentum': momentum, 'branch': branch, 'star': star, 'separation_factor': float(separation_factor), 'normalized': bool(normalize), 'core_diagnostics': core_diagnostics, 'density_diagnostics_raw_space': diagnostics_raw, 'base_clusterings': int(base_clusterings), 'base_k_min': int(base_k_min), 'base_k_max': int(base_k_max), 'base_strategy': base_strategy, 'base_info': base_info, 'point_distribution': pd}
...
def get_presets() -> dict[str, dict]:
    return {'densired_compact': {'dim': 2, 'clunum': 8, 'core_num': 120, 'data_num': 3000, 'seed': 6, 'domain_size': 30.0, 'epsilon': 0.55, 'step_ratio': 0.7, 'min_pts': 8, 'noise_ratio': 0.05, 'density_factors': [1.0, 1.0, 0.8, 0.7, 1.2, 1.1, 0.9, 1.3], 'momentum': 0.0, 'branch': 0.0, 'star': 0.0, 'separation_factor': 1.15}, 'densired_stretched': {'dim': 10, 'clunum': 8, 'core_num': 140, 'data_num': 3500, 'seed': 6, 'domain_size': 45.0, 'epsilon': 0.65, 'step_ratio': 0.75, 'min_pts': 8, 'noise_ratio': 0.05, 'density_factors': [1.0, 1.0, 0.7, 0.6, 1.4, 1.2, 0.9, 1.3], 'momentum': 0.85, 'branch': 0.05, 'star': 0.0, 'separation_factor': 1.2}, 'densired_branching': {'dim': 2, 'clunum': 8, 'core_num': 160, 'data_num': 3500, 'seed': 6, 'domain_size': 35.0, 'epsilon': 0.55, 'step_ratio': 0.7, 'min_pts': 8, 'noise_ratio': 0.08, 'density_factors': [1.0, 1.0, 0.7, 0.6, 1.4, 1.2, 0.9, 1.3], 'momentum': [0.5, 0.75, 0.8, 0.3, 0.5, 0.4, 0.2, 0.6], 'branch': [0.05, 0.1, 0.15, 0.05, 0.05, 0.15, 0.1, 0.2], 'star': [0.0, 0.2, 0.6, 0.0, 0.15, 0.0, 0.2, 0.4], 'separation_factor': 1.2}}
