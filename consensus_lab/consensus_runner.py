"""Общая инфраструктура прогона консенсус-алгоритмов.

Выделяет повторяющийся паттерн из всех четырёх `run_*` функций:
загрузка → валидация → субдискретизация ансамбля → цикл прогонов →
сбор метрик → формирование результата.
"""
...
def _build_bc_idx(
    ...
    """Построить матрицу индексов базовых кластеризаций для каждого прогона."""
...
def run_consensus_loop(
    ...
    build_consensus_fn: Callable[[np.ndarray, np.ndarray, int], np.ndarray],
    *,
    ...
    qd_alpha: float = 0.5,
    clamp_m_name: str | None = None,
) -> dict:
    """Единый цикл прогонов для алгоритмов, работающих с (members, gt).

    Args:
        dataset_path: путь к .mat/.npz.
        build_consensus_fn: функция (base_cls, gt, m) → consensus_matrix (n×n).
            Вызывается на каждом прогоне с подвыборкой базовых кластеризаций.
        clamp_m_name: если задано, усекает m до pool_size с предупреждением.

    Returns:
        Словарь с метриками, совместимый с ConsensusResult.
    """
    ...
    out = {
        ...
        "f_scores": f_scores,
        "selected_base_clusterings": bc_idx[0].astype(int).tolist(),
        "method": method,
        "m": int(m),
        "runs": int(cnt_times),
        "seed": int(seed),
        ...
        "qd_alpha": float(qd_alpha) if selection_strategy == "qd" else None,
    }
    if extra_per_run:
